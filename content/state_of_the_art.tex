...
similar to omniscient and repay-based debuggers. A novel case of this type of debuggers, for JavaScript programs, is DeloreanJs~\cite{leger23}. It has three main features: the ability to navigate 
...
execute a new timeline with the new values, available in timepoint debuggers as DeloreanJS, 
is desirable in the context of \ac{RL} programs due to the possibility of exploring different 
configurations as part of the same execution session. We take inspiration from such features,
and in particular DeloreanJS, as a reference to create our debugger for \ac{RL} programs.
...
(the standard Python bytecode interpreter), implemented by several IDEs that provide a \ac{GUI} for 
the debugger. \ac{PDB} is a powerful interactive source code debugger for Python programs, 
...
\ac{PDB} built on top of the PyPy interpreter, allowing developers to step forward and backward in 
