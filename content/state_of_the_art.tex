...
We first present the state of the art in debugging approaches and representative tools in different 
...
(either stepping into/through the instructions, or in a continuous execution).

\ac{GDB}~\cite{stallman11} is a breakpoint-based source-code compiler, mainly used C and C++ 
(although it also supports other programming languages, like Ada, Go, or Rust). GDB helps 
...
The omniscient debugger for Java~\cite{pothier07} records all events that take place during the 
execution creating a trace history. JARDIS~\cite{barr16} is an omniscient debugger for JavaScript 
tracing web application events. JARDIS offers a GUI to navigate the recorded trace. Other JavaScript 
...
with the exact same behavior they originally executed from given checkpoints in the program.

Jokey~\cite{saito05} enables replay-based debugging for C. With Jockey developers can replay 
program executions to analyze program behavior from specific checkpoints. 
QueryPoint~\cite{mirghasemi11} is a Javascript replay-based debugger let developers to go back to 
...
similar to omniscient and repay-based debuggers.

DeloreanJs~\cite{leger23} is novel debugger implementing the ideas of timepoint debuggers for 
JavaScript programs. DeloreanJs has three main features: the ability to navigate 
