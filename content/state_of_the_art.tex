...
This section presents the state of the art from three perspectives: (1) the main debuggers in other 
programming languages, (2) existing Python debuggers, and (3) debuggers for \ac{AI} programs in general, 
...
\sidemargin{par:other-langs}{{\fref{rem:other-langs}}}
...
However, there are also standalone debugging tools that offer more advanced features and capabilities than the 
built-in debugging tools of IDEs. Here we present debugger representatives according to the 
...
(either stepping into/through the instructions, or in a continuous execution). For instance, \ac{GDB}~\cite{stallman11} is a breakpoint-based source-code compiler, mainly used for C and C++ programs (although it also supports other programming languages, like Ada, Go, or Rust). GDB helps 
...
execution to step both forward and back from a breakpoint.

\item[Omniscient debuggers.] Omniscient debugging (also back-in-time or reversible debuggers) 
records the execution history of a program and let developers to explore the execution stepping 
forward and back from any given execution point (\ie breakpoint or program state)~\cite{hofer06}.  A representative example is the omniscient debugger for Java~\cite{pothier07}, which records all events that take place during a program execution and then creates  a trace history. JARDIS~\cite{barr16} is an omniscient debugger for JavaScript 
...
from running applications~\cite{trackjs, raygun}. Flow-centric back-in-time debugger~\cite{lienhard09} 
is a SmallTalk debugger that that on top of enabling step-backs, it also enables the possibility to 
explore additional execution contexts.

\item[Replay-based debuggers.] Replay-based debuggers let developers to re-execute programs 
with the exact same behavior they originally executed from given checkpoints in the program. For example,  with Jokey~\cite{saito05} C developers can replay program executions to analyze program behavior from specific checkpoints. Another example is
...
\item[Timepoint debuggers.] Timepoint debugging constitute a combination of the previously 
...
similar to omniscient and repay-based debuggers. A novel case of this type of debuggers, for JavaScript programs, is DeloreanJs~\cite{leger23}. It has three main features:  navigate 
...
a full-featured debugger, as it does not allow you to change the values of variables, nor execute 
...
framework. 
\sidemargin{par:fuzzing}{{\fref{rem:fuzzing}}}
