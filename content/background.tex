...
from $s$ to a new state $new\_s \in S$. Additionally, associated to the execution of the action $a$ at 
...
agent is to maximize its total reward, achieve by iteratively updating the reward function associated 
with with every state-action pair until converging to the maximum values. \fref{lst:qlearning} presents 
...
debug a program. 
An improvement constitutes the use of traces or logs~\cite{yin20}, in which variables values and 
