...
process~\cite{telles01,zeller09,spinellis16,agans23}, including tasks to understand the code, identify and reproduce errors, isolate the problem, and fixe the problem, 
...
A classic manual debugging technique is the use of traces in the code through printing the values of 
several variables as the program is executed. However, this is not an efficient or scalable way to 
debug a program. 
An improvement over the use of traces is the use of logs~\cite{yin20}, in which variables values and 
specific events are recorded into an external log, to then be analyzed after the program execution; however, it requires the complete execution of the system (or until a crashing event 
occurs), to then analyze  variables and their values for correctness.
An alternative to post-mortem analysis is the use of debuggers, which allow developers to inspect the 
