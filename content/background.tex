...
of the Q-learning algorithm, the environment is composed of a set of states $S$, and a set of actions 
$A$ the agent can execute. By executing an action $a\in A$ at state $s \in S$, the agent transitions 
from $s$ to a new state $new\_s \in S$. Additionally, associated with the execution of the action $a$ at 
state $s$, the agent receives a reward $R(s,a)$. The agent's learning is driven by the choosing of the 
action to execute, which corresponds to the action that maximizes its total reward. The goal of the 
agent is to maximize its total reward, achieved by iteratively updating the reward function associated 
with every state-action pair until converging to the maximum values. \fref{lst:qlearning} presents 
the base algorithm for Q-learning~\cite{sutton18} in pseudo-code.

\begin{python}[
  caption={Pseudo code for the Q-Learning algorithm.}, label={lst:qlearning}]
 `\textbf{Require:} An environment with states $S$, actions $A$, and reward function $R(s, a)$`
 `\textbf{Require:} A learning rate $\alpha \in [0, 1]$`
 `\textbf{Require:} A discount factor $\gamma \in [0, 1]$`
 `\textbf{Require:} Exploration strategy (\eg $\epsilon$-greedy)`
# Initialize Q-table `$Q(s, a) \gets 0$` for all `$s \in S$, $a \in A$`
foreach episode:
   `$s_0$` # Initialize the starting state 
   foreach time_step in episode:
      if exploration_step:
         a = random(`$\epsilon$`) # Choose an action `$a$` using the exploration strategy (e.g., `$\epsilon$`-greedy)
      else
         a = `$\arg\max_a Q(s, a)$`  # Choose an action `$a$` based on the current Q-values
      endif
        R(s,a), new_s = action(a) # Take action `$a$`, observe reward `$R(s,a)$` and next state new_s
        #Update Q-value: 
        `$Q(s, a) \gets Q(s, a) + \alpha \left( R(s,a) + \gamma \max_{a'} Q(s', a')\right)$`
        s = new_s # Set s `$\leftarrow$` new_s
    endfor
endfor
\end{python}

\ac{RL} has gained wide popularity in the last couple of years, finding applications in a wide range of 
domains~\cite{beakcheol19}, particularly in the control of industrial process~\cite{kiumarsi18} 
(improving the performance of the on-line learning control system or optimizing temperature control 
and power consumption), computer networks~\cite{alrawi13} (improving adaptability of Wireless 
Sensor Networks to changing situations and eliminating the need for system redesign), and 
robotics~\cite{zhang15} (by providing frameworks and toolkits for designing sophisticated behavioral 
aspects). 
...
type of bug of interest for our work is runtime errors, in which programs compile, but present  
...
process~\cite{telles01,zeller09,spinellis16,agans23}, including tasks to understand the code, identify and reproduce errors, isolate the problem, and fix the problem, 
...
in which developers state a hypothesis about the expected behavior of the system, and then confirm 
its veracity. A consequence of this is that whenever the hypothesis 
about the program behavior is not true, developers must identify the bugs causing the problem, 
and fix them. Moreover, we focus on the use of debuggers to support the process of identifying bugs.

A classic manual debugging technique is the use of code check-points through printing the values of 
several variables as the program executes. However, this is not an efficient or scalable way to 
debug a program. 
An improvement is the use of traces or logs~\cite{yin20}, in which variables values and 
specific events are recorded into an external log, to then be analyzed after the program execution; 
however, it requires the complete execution of the system (or until a crashing event 
occurs), to then analyze  variables and their values for correctness.
Debuggers can be used as an alternative to post-mortem analysis, allowing developers to inspect the 
state of programs while they execute, offering control over the execution. Debuggers are essential 
tools for software development, as they help developers to identify and fix bugs in their programs. 
There are many types of debuggers, each with its own set of features and capabilities, but usually 
the main operations offered by debuggers~\cite{matloff08} are:
...
\begin{itemize}
    ...
    is seldom available across debuggers.
\end{itemize}
