...
independent program module. Common to software development processes, the development of 
\ac{RL} programs, is prone to (1) errors, (2) unexpected behavior in the application logic like incorrect 
state values, and (3) erroneous logic implementations. Moreover, within \ac{RL} programs also appear 
...
First, like for \ac{ML} programs, \ac{RL} algorithms are usually used as a black box, calling an external 
library to manage learning (usually accesible/implemented to developers). This means that 
...
behavior directly, without having to incur in long train-test processes for each possible configuration. 
With \flik, developers can try a particular agent configuration, go back in the execution, try another 
configuration and observe if the behavior is appropriate. Such capabilities are useful in pinpointing 
the root cause of undesired behavior. 
...
The reminder of the paper is structured as follows. \fref{sec:background} presents the background 
definitions of \ac{RL}, the Q-learning algorithm, bugs, and debuggers. \fref{sec:state_of_the_art} 
presents the state of the art with 
...
Finally, \fref{sec:conclusion} presents the conclusion of our work and avenues for future work.
