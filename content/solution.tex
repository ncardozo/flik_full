...
CPython interpreter's tracing architecture. Once stopped, \ac{PDB} has access to the execution 
...
the step back commands (\spy{step_back} or \spy{back_to}) stops the current execution an moves 
...
sate, we use the \spy{exec} Python function that receives the stored \spy{f_globals} and \spy{f_locals} 
