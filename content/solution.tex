...
function. From the top, in the first frame in the blue rectangle (Execution frame) there is the running 
execution.  The second frame (in the purple rectangle - Source code frame) shows the source code being debugged; note that the result of executing Line 018 (a print statement for the array to be sorted) is displayed in the Execution Frame. The third frame (in the red rectangle - Variables frame) shows the variables 
used in the program so far. Finally, at the bottom of the interface we have the interactive console in 
...
Having introduced the features of \flik and their use to debug \ac{RL} programs using the different 
...
\flik is a console-based debugger built on top of \ac{PDB}, by adding features such as colored syntax 
...
Thus, \flik works with external libraries and capture stdout 
