...
commands, we now turn our attention to \flik's implementation and its inner workings. 
...
consequence, the buffer keeps the YYY most recent history frames, removing the oldest frame in the 
...
for a specific execution line within the saved metals (when \spy{back_to} is used).
...
A direct consequence of restoring a previous program state is that the execution resumes is a new 
...
any of the stored variables (using the \spy{set vat} command). At a given execution point, updating 
...
circumstances. Using this feature it is possible to modify learn values or learning parameters in 
