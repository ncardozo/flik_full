...
of a buffer of fixed size (XXXMB in our current setting) as shown in \fref{fig:flik-architecture}
\sidemargin{par:state}{{\fref{rem:state}}}

\paragraph{Capture system state.}
Stores the state in each execution step (\ie action taken by the agent). \flik saves the local 
...
history, and their corresponding location in the code. 
     ...
     Restores a previous program state as, from the previously stored information, from the 
    ...
    order to re-create an alternative execution path. 
     ...
     Finally, connecting the previous two features, is the action of stepping back. This feature is 
