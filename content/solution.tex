...
The black box behavior of \ac{ML} and \ac{RL} agents posits a problem to understand agents and 
their behavior, especially if unexpected behavior is observed. We put forward debuggers as an 
appropriate tool to understand agents' behavior. Moreover, debuggers with the possibility to modify 
programs' state and continue execution in new execution paths exploiting the modified values are 
even more appropriate to understand \ac{RL} agents, given their intrinsic nature of continuous 
interaction with the environment. Such property enables developers with the possibility to experiment 
over the behavior of agents without having to go over the extensive process of stopping agents and 
going through the complete exploration with the new values.

Currently, most of the debuggers are postmortem, or do not allow developers to go back in time to 
replay and analyze the program state. Additionally, most debuggers do not allow developers to 
...
The \flik back-in-time debugger is based on two main features:
\begin{description}
    ...
    avoiding the stop-modify-re-run loop which causes to lose the program state, or the training data 
    already accumulated. Such feature will help in identifying the root cause of erroneous agent 
...
These features aim at tackling the problem of interaction with the program, as it allows to inspect 
the behavior of a program with respect to different variable values. Additionally, developers may 
interact with the program by stepping through the execution, both forward and backward, observing 
the effects of specific interactions between the agent and the environment. The capacity to explore 
the execution allows developers to evaluate agents' behavior and the quality of the \ac{RL} programs, 
helping to improve the development of these programs.
...
on different execution paths. Specifically, for the case of \ac{RL} agents this means that using \flik it is 
...
\sidemargin{par:requirements2}{{\fref{rem:requirements}}}
We now turn our attention to the implementation of \flik and its inner workings. \flik is a console-based 
debugger, constructed over the \ac{PDB} debugger, as shown in the general \flik architecture in 
