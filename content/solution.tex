...
In order to contribute to the development of \ac{RL} programs, 
...
stopping-modifying-starting-training \ac{RL} agents to evaluate different learning parameters or 
algorithm values, the \flik back-in-time debugger is based on two main features:
\begin{description}
    \item[Stepping back:] Due to the lengthy execution loop in \ac{RL} programs, we 
    wanted to have a functionality that will allow developers to step back the execution to observe the 
    ...
    Stepping back will let developers interact directly with the program, 
...
variables, values, environment, states, and rewards. 
...
\fref{fig:debugger} shows \flik's interface working to debug an example program for the 
\spy{bubble_sort} function. From the top, in the first frame in the blue rectangle (Execution frame) 
there is the running execution.  The second frame (in the purple rectangle - Source code frame) 
shows the source code being debugged; note that the result of executing Line 018 (a print statement 
for the array to be sorted) is displayed in the Execution Frame. The third frame (in the red rectangle - 
Variables frame) shows the variables used in the program so far. Finally, at the bottom of the interface 
we have the interactive console in which developers can send commands to \flik to effectively debug 
the program.

We now use a running example to present the features and functionality/commands of \flik. For 
...
commands (in \fref{tab:flik-commands}) used to detect a bug in the code. Running the program, we 
pay special attention to the \spy{run} function, which we can access using the \spy{n} command until 
we reach it. Alternatively, it 
...
the value of the \spy{action} variable (\spy{p action}), to observe how different actions are taken, 
...
Having introduced the features of \flik and their use to debug \ac{RL} programs through the different 
commands, we now turn our attention to \flik's implementation and its inner workings.\footnote{\flik's distribution is available as a Docker image at: \url{https://hub.docker.com/r/lrodriguez22/py-flik-debugger}.}  
...
instrument the code of a running interpreted program by means of the \spy{breakpoint()} \ac{PDB} 
command. The instrumentation hooks directly to the evaluation loop of the Python VM using the 
CPython interpreter's tracing architecture. Once stopped, \ac{PDB} has access to the execution stack 
frame, the VM event (with the information of the line to execute, the function to call, the value to 
return or an exception to throw), and the \spy{arg} context variable. Using the instrumentation 
available for Python programs makes \flik work with the full Python specification without having to 
build an interpreter and program instrumentation from scratch. 
Thus, \flik works with external libraries and capture \spy{stdout}
...
Storing the program state is a key functionality to realize \flik's main two features. As mentioned 
previously, \flik sequentially stores the system state for each execution step (\ie action taken by the 
agent) in a history buffer. 
The information saved in the history buffer includes all global and local variables' values after the 
execution of the instruction, together with metadata related to the executed expression, as for 
example its line number. System state is encapsulated in stack frames in Python, which can be 
captured using the \spy{inspect.stack()} function. Each stack frame contains different information 
related to the execution state of a function call, for which we save:
\begin{itemize}
    \item \spy{f_locals}: A dictionary of local variables within the current execution frame.
    \item \spy{f_globals}: A dictionary of global variables accessible within the current execution frame.
    \item \spy{f_code}: A code object representing the executing function's bytecode and source code metadata.
    \item \spy{f_lineno}: The current line number of the function being executed.
\end{itemize}
...
functions we restrict the history buffer to have a fixed size of over 10,000 frames in our current setting 
(this parameter is configurable, although managing a larger number of frames hinders the 
responsiveness of \flik). As a consequence, the buffer keeps the 10,000 most recent 
history frames, removing the oldest frame in the history when the buffer is full.
The use of the history buffer has an impact on the memory usage of \flik, increasing the overall 
memory used by the program. However, the memory increase is constant and will not go over 10,000 
frames more than the memory of an \ac{RL} program not using \flik.

\paragraph{Step-back.}    
The history buffer is used to enable stepping back in the program execution. In an execution function, 
the step back commands (\spy{step_back} or \spy{back_to}) stop the current execution and move 
the execution to a previous frame saved in the buffer. 
When \spy{step_back} is used, the execution moves the program to the previous saved state (\ie the 
last executed command in the program).  When \spy{back_to} is used, \flik searches through the 
buffer for the first instance of the specific execution line within the saved metadata.

\paragraph{Restore program state.}    
Once the program execution is at a specific previous frame in the history buffer, to resume the 
execution from that given point, \flik restores the saved state (\ie local and global variables, current 
line number, and executing file details as the program counter) as the new program state. Restoring a 
program state sets back all local and global variables' values, and their associated execution context, 
to the values stored at the current frame in the history buffer.

A direct consequence of restoring a previous program state is that the execution resumes in a new 
execution path, and the execution path before the step-back is lost. That is, all state changes and 
side-effects observed are ``lost'', and the new execution path creates its own new state and 
side-effects using the new program state. Currently \flik does not keep track of the different 
execution paths, as there is no clear way to navigate them without the use of a GUI. To restore the 
state, we use the \spy{exec} Python function that receives the stored \spy{f_globals} and \spy{f_locals} 
variables as parameters. \spy{exec} then executes a list of stored state snapshots (line number 
...
Note that the use of the history buffer lets \flik manage concurrent execution or reversible operations 
without any additional process. While \flik directly does not deal with concurrent execution, as this is 
not used in \ac{RL}, it does not pose a problem. Executed functions are recorded sequentially in the 
order they occur in the history buffer, stepping back to a function just reverts the state to the point 
before the function execution and resumes execution from that point on, therefore there is no need to 
keep track of concurrent executions. Second, \flik is not a reverse debugger, as it does not roll-back 
on previously executed function. Rather the saved frame in the history buffer records all variables' 
values, function, and line metadata, so that when execution can resume from exactly the same state 
when first executed, even in the face of I/O operations.
...
\flik creates a causual connection between the Python variables stored in memory and the variables 
