...
\end{remark}
\begin{solved}
We did a full revision over the paper and cleaned up the errors suggested by the reviewer and other mistakes we found to improve the overall presentation. Additionally, we restructure the main four sections of the paper to increase the clarity of the presentation
\end{solved}
...
\end{remark}
\begin{solved}
We modified the argument to remove the confusion, stating that such libraries are used off-the-shelf plugged into an application, usually leaving that part of the agent's behavior overlooked, specially in the case those modules are black-boxes developers do not have access to. Without access it is not possible to observe the fine-grained behavior of such libraries.

We also clarified this point in \fref{sec:solution} where a similar argument was present
\end{solved}
...
\end{remark}
\begin{solved}
We added an explicit mention of the representation of memory in \flik (\fref[vario]{par:state}) and the way it works to capture programs' state in the following presentation of \flik's functionality
\end{solved}

\begin{remark} \label{rem:python-io}
What happens if there are I/O operations?
\end{remark}
\begin{solved}
We clarified that \flik can work as the normal \ac{PDB} debugger when using standard I/O operations in \fref{sec:flik-internals} (\fref[vario]{par:python})
\end{solved}

\begin{remark} \label{rem:python-full}
Does it accept 100\% of Python? 
\end{remark}
\begin{solved}
\flik accepts all of Python being built on top of PDB, which already accepts all of Python. This is clarified in \fref{sec:flik-internals} (\fref[vario]{par:python})
\end{solved}

\begin{remark} \label{rem:python-lib}
what if there are calls to libraries whose source code isn't available?
\end{remark}
\begin{solved}
We added the text explaining what happens when external libraries are used, either with available code, or unavailable code.
\end{solved}
...
\end{remark}
\begin{solved}
We added a discussion about memory overhead and scalability of using \flik according to the tool's its design as described in the new section discussing how to capture the state (\fref[vario]{par:scalability}).
\end{solved}
...
\end{remark}
\begin{reconsider}
We opted to leave the description of each of the python approaches as their single paragraph, in correspondence with the Sections 3.1 and 3.3
\end{reconsider}
...
\begin{remark}\label{rem:fuzzing}
What is the "Framework Fuzzing"? Explain and add some reference.
\end{remark}
\begin{solved}
We added a clarification about how  fuzzing works in the framework (\fref[vario]{par:fuzzing}).
\end{solved}
...
\begin{remark}\label{rem:one-bug}
Just consider adding a bug? In practice, I guess, programs can contain several bugs that often interact with each other.
\end{remark}
\begin{solved}
We clarify that indeed while for each evaluation scenario we introduce a single bug, others could exist in the general program and \flik would work just as well. (\fref[vario]{par:one-bug})
\end{solved}
...
\end{remark}
\begin{solved}
We used the research design book as a guide to describe the evaluation study as a whole, but to structure and describe the research design and study structure and steps. This description was added as part of the new section describing the Study Design, \fref{sec:design} 
\end{solved}
...
\end{remark}
\begin{solved}
We removed this erroneous argument of the paper, and leave in the argument about the cost of training an agent until its behavior emerges. evaluating different parameters an values becomes very costly  due to the training execution steps of the agent. A debugger that let's developers try out different values while running, is more appropriate for such cases.
\end{solved}
...
\end{remark}
\begin{solved}
Indeed we meant the use of a GUI, and fixed the table accordingly
\end{solved}

\begin{remark}\label{rem:postmortem}
...
\end{remark}
\begin{solved}
Our phrasing about the debuggers was incorrect causing confusion. We rephrase the sentence to express that many debuggers allow only to observe the behavior based on the state of variables once a statement executes. In contrast to the back-in-time features of \flik (\fref[vario]{par:postmortem})
\end{solved}

\begin{remark}\label{rem:implementation2}
...
\end{remark}
\begin{solved}
We related \flik with the state of the art in the restructuring of \fref{sec:solution} in response to the following Remarks: \ref{rem:implementation}, \ref{rem:features}, \ref{rem:features2}, \ref{rem:implementation2}, \ref{rem:delorean}
\end{solved}

\begin{remark} \label{rem:features}
...
\end{remark}
\begin{solved}
We rephrased the statement of the features available in \flik as following. First, we make it clear that \flik is the combination of the two features described in \fref{sec:flik-nutshell} (\ie stepping back, and timepoints). In the description of each of the features we explicitly mention where the features are adopted from. Second, we changed the description of the main features to \flik's functionality, to avoid confusion about the existence of features. Just the two features realized through the functionality to store program's data, stepping back, and restoring saved data. The functionality is described in detail in the new \fref{sec:flik-internals}. 
\end{solved}

\begin{remark}\label{rem:features2}
...
\end{remark}
\begin{clarify}
This isn't actually a new feature, this is the second feature presented at the beginning of \fref{sec:flik-nutshell}, phrased a little different. To avoid the confusion, we have repressed the original presentation of the two main features in \flik, aligning with the description of modification of execution paths.
\end{clarify}
...
\end{remark}
\begin{solved}
In accordance to the response to \fref{rem:implementation}, we introduce the new \fref{sec:flik-internals} to explain the implementation details and design of \flik (\fref[vario]{par:requirements2})
\end{solved}

\begin{remark} \label{rem:delorean}
...
\end{remark}
\begin{solved}
We explain in more depth the design and the decision around \flik with the importance of the developed features for the specific case of \ac{RL} as part of the new section \fref{sec:flik-internals}.
\end{solved}

\begin{remark}\label{rem:experimental-design}
The experimental design, including the kind of experiment, is not clearly stated and not sufficiently explained.
\end{remark}
\begin{solved}
We completely re-structure \fref{sec:evaluation} to present the study design and to answer Remarks \ref{rem:experimental-design}-\ref{rem:study-variables}, \ref{rem:survey}.

In particular we added \fref{sec:design} to explicitly describe the study design, as the description was spread over different parts of Sections \ref{sec:evaluation} and \ref{sec:results}. (\fref[vario]{par:experimental-design})
\end{solved}

\begin{remark}\label{rem:rqs}
...
\end{remark}
\begin{solved}
We added the missing research questions to \fref{sec:evaluation} as a means to set-up the study (\fref[vario]{par:rqs})
\end{solved}

\begin{remark} \label{rem:study-conditions}
...
\end{remark}
\begin{solved}
As part of \fref{sec:design} we included a paragraph describing the ordering and complexity of the tasks further than the initial description at the beginning of \fref{sec:evaluation}. (\fref[vario]{par:study-conditions}).

Additionally, we discussed the ordering of the tasks and its influence on the results as part of the newly introduced threats to validity section (\fref{sec:threats}) in the discussion about the construct validity.
\end{solved}

\begin{remark} \label{rem:study-variables}
...
\end{remark}
\begin{solved}
We added a paragraph introducing the variables in the evaluation, explaining what we measure which each of them, and relating them to the specific research question they answer. (\fref[vario]{par:study-variables}).
\end{solved}
...
\end{remark}
\begin{solved}
In answering the remark related to the results and the analysis (\ref{rem:time}, \ref{rem:characterization}, \ref{rem:characterization2}, and \ref{rem:results}) we completely restructured \fref{sec:results} to make the results, explanations, comparisons, and answers to the research questions more explicit.

We added the contrast between the effective resolution of each task and the time it took more explicitly to the new \fref{sec:analysis}. Then in the new \fref{sec:answers}, we use the explicit comparisons of measured time and success to provide an answer to \ref{enum:usefulness}
\end{solved}

\begin{remark} \label{rem:survey}
...
\end{remark}
\begin{solved}
We discuss this factor in the threats to internal validity section, explaining that while the risk was note foreseen, it did not present itself and the feedback and comments received by the users was relevant and of quality.
\end{solved}
...
\end{remark}
\begin{solved}
The expertise level is self-reported taken the characterization of the questionnaire. However, as we did not record the years of experience in working with Python we included a characterization of the  demographic for the participants in \fref{sec:general-knowledge}. As all participants have taken the \ac{RL} course, and this course is an advance course within the master, they have had at least a year of experience in programming and Python. Most of the students coming from the engineering bachelor have an additional four years of experience.
\end{solved}

\begin{remark} \label{rem:characterization2}
...
\end{remark}
\begin{clarify}
While we clarify the experience of participants in using debuggers in \fref{sec:general-knowledge}, we have no evidence that can confirm a reason of the self reported inexperience. 

One possible explanation is, as the reviewer correctly identifies, that the background of participants is not specifically in software engineering or computer science. Some of the participants indeed come from different backgrounds within engineering or even business school, but even then they have had programming experience and taken the basic cs programming in Python course. However another explanation for this is that the practice of debugging is not as well spread now a days, due to proliferation of scripting languages (as Python or JavaScript) in which log-based debugging is a common practice. Moreover, the use of alternative tools like LLMs for automatic completion and programming shepherding may also have an impact in the current use of debuggers
\end{clarify}
...
\end{remark}
\begin{solved}
We extended the results section provided a deeper analysis on the results and in particular the answers to the research questions, which should provide the required insights about  the usefulness of \flik.
\end{solved}
...
\end{remark}
\begin{solved}
We extended the discussion with more perspectives based on the answers to the research questions and future perspectives.
\end{solved}

\begin{remark}\label{rem:threats}
...
Furthermore, we added a threads to validity section, \fref{sec:threats} (\fref[vario]{par:threats}), to discuss the generalization and possible bias that may arrive from our experiments in order to reduce any questions of doubts about the results.
\end{solved}

\begin{remark} \label{rem:ethics}
...
\end{remark}
\begin{solved}
We added a paragraph describing there has been an evaluation and approval from the ethics board at the university, and the components included in the ethics evaluation, as well as the existence and collection of the concentration form (\fref[vario]{par:ethics}).
\end{solved}
...
\end{remark}
\begin{solved}
The conclusion about the usability of \flik has been addressed as part of the answer to Remarks \ref{rem:experimental-design} and \ref{rem:characterization} - \ref{rem:threats}.
\end{solved}
...
\end{remark}
\begin{solved}
We clarified, that the motivation to the future work by making explicit the mention that these come from our own experience developing \flik, or from the comments received by participants to the study (as part of the questionnaire), as this may have been unclear from the previous version of the text.

We additionally added the possibility to explore the ideas for omniscient debuggers already explored by~\citet{pothier07} (as suggested by the reviewer), and keep the or ideas of using layered cashing we had express before.

Finally, the idea of the future work is to improve \flik, even if this does not present an innovative idea (which it might, the work needs to be done first). However we argue that the avenues of future work are still of importance to the user community, which would benefit from better and more efficient tools.
\end{solved}
