...
\end{remark}
\begin{solved}
We rewrote the whole introduction to have a more abstract presentation of the motivation, and reduce its technicality (\fref[vario]{par:intro}). All the technical details have been moved to a new sections providing the background on \ac{RL} (\fref{sec:background} - \fref[vario]{par:background})  
\end{solved}
...
\end{remark}
\begin{solved}
We rephrased the sentence to a meaningful definition of defining an agent to reach a goal in an environmen
\end{solved}
